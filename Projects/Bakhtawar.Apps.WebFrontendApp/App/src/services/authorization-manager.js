    ...
    try {
      ...
      try {
        if (this._popUpDisabled) {
          throw new Error('Popup disabled. Change \'authorization-manager.js:AuthorizationManager._popupDisabled\' to false to enable it.');
        }
...
export const addAuthorizationHeader = async (operation, forward) => {
  const isAuthenticated = await authorizationManager.isAuthenticated();
  const token = isAuthenticated
    ? await authorizationManager.getAccessToken()
    : '';
